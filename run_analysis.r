## this will generate a tidy dataset according to the requirments
...
train<-cbind(subj_train,X_train_sel,Activity=activities$Activity)
...
test<-cbind(subj_test,X_test_sel,Activity=activities$Activity)
dataset<-rbind(train,test)
write.table(dataset,"getdata-projectfiles-UCI HAR Dataset\\getdata.txt")

## this will create a separate dataset with the mean of all variables

Avg<-aggregate(dataset[,-c(1,75)],by=list(Subject=dataset$Subject,Activity=dataset$Activity),FUN=mean)
write.table(Avg,"getdata-projectfiles-UCI HAR Dataset\\Average.txt")
